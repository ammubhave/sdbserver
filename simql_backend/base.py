...
try:
    ...
    raise ImproperlyConfigured("Error loading simqldb modules: %s" % exc)
...
class DatabaseFeatures(BaseDatabaseFeatures):
    ...
    def uses_savepoints(self):
        return False# Database.sqlite_version_info >= (3, 6, 8)
...
class DatabaseOperations(BaseDatabaseOperations):
    ...
    def bulk_insert_sql(self, fields, num_values):
        ...
        raise Exception('bulk isnert sql\r\n' + str(fields) + '\r\n' + str(num_values))
...
class DatabaseWrapper(BaseDatabaseWrapper):
    ...
    def get_connection_params(self):
        ...
        if not settings_dict['URL']:
            from django.core.exceptions import ImproperlyConfigured
            raise ImproperlyConfigured(
                "settings.DATABASES is improperly configured. "
                "Please supply the URL value.")
        kwargs = {
            'database': settings_dict['URL'],
            #'detect_types': 3,# Database.PARSE_DECLTYPES | Database.PARSE_COLNAMES,
        }
